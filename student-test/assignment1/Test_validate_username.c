...
void test_validate_my_username()
{
     ...
     */
     // Step 1: Call the my_username() function to get the hard-coded username
    const char* hardcoded_username = my_username();

    // Step 2: Call the malloc_username_from_conf_file() function to get the username from the config file
    char* config_username = malloc_username_from_conf_file();

    // Step 3: Use TEST_ASSERT_EQUAL_STRING_MESSAGE to verify the two strings are equal
